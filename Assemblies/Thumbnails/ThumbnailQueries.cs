...
namespace Thumbnails
{
    public static class ThumbnailQueries
    {
        ...
        /// <summary>
        /// Clears all cached thumbnail URLs for a user to force fresh rendering.
        /// </summary>
        /// <param name="connectionString">Connection string to Postgres database.</param>
        /// <param name="userId">User id.</param>
        /// <param name="cancellationToken">Cancellation token.</param>
        /// <returns>True if successful, false if user not found.</returns>
        public static async Task<bool> ClearUserThumbnailUrlsAsync(string connectionString, long userId, CancellationToken cancellationToken = default)
        {
            if (string.IsNullOrWhiteSpace(connectionString))
                throw new ArgumentException("connectionString is required", nameof(connectionString));
            if (userId <= 0)
