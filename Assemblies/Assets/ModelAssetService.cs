...
namespace Assets
{
    public sealed class ModelAssetService
    {
        ...
        public ModelAssetService(IConfiguration? configuration = null)
        {
            _assetService = new AssetService(configuration ?? new ConfigurationBuilder().Build(), null);
        }

        public async Task<long> CreateModelAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!isValid)
                throw new ArgumentException(errorMessage ?? "Model validation failed.");
            ...
            var modelAssetId = await _repository.CreateAssetAsync(connectionString, createParams, cancellationToken)
                .ConfigureAwait(false);

            await _userAssetsRepository.AddUserAssetAsync(connectionString, ownerUserId, modelAssetId, cancellationToken)
                .ConfigureAwait(false);

            // Render thumbnail using the centralized AssetService (fire-and-forget)
            _ = Task.Run(async () => await _assetService.RenderAssetThumbnailAsync(modelAssetId, cancellationToken).ConfigureAwait(false));
