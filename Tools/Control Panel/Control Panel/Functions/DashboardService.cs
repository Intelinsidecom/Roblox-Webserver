    ...
    public class DashboardService
    {
        ...
        private async Task GetFrontendDataAsync(DashboardData data)
        {
            try
            {
                ...
                var apiStatus = await _apiServiceQueries?.GetApiStatusAsync() ?? new ApiStatus { IsOnline = false, Status = "Not Configured" };
                var cdnStatus = await _cdnQueries.GetCdnStatusAsync();
                var rccLogStatus = await _rccLogQueries?.GetServiceStatusAsync() ?? new ServiceStatus { IsOnline = false, Status = "Not Configured" };
                var websiteServiceStatus = new WebsiteServiceStatus
                {
                    LastChecked = DateTime.Now,
                ...
                if (websiteStatus.IsOnline && 
                    (_apiServiceQueries == null || apiStatus.IsOnline) && 
                    cdnStatus.IsOnline)
                {
                    websiteServiceStatus.Status = "Healthy";
                    websiteServiceStatus.OverallIsOnline = true;
                }
                else
                {
                    websiteServiceStatus.Status = "Unhealthy";
                    websiteServiceStatus.OverallIsOnline = false;
                }
