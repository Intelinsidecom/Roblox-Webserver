        ...
        public async Task<IActionResult> GetGameName(long id, string name, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                if (isUserAuthenticated)
                {
                    ...
                    try
                    {
                        var profileData = await UserQueries.GetUserProfileDataAsync(connStr, userId).ConfigureAwait(false);
                        if (profileData != null)
                        {
                ...
                ViewBag.UserMembershipLevel = userMembershipLevel;
                long userRobuxBalance = 0;
                if (isUserAuthenticated)
                {
                    try { userRobuxBalance = await UserQueries.GetCurrencyByIdAsync(connStr, userId, "robux").ConfigureAwait(false); } catch { }
                }
                ViewBag.UserRobuxBalance = userRobuxBalance;
                ...
                ViewBag.GamePassCount = await GamesRepository.GetGamePassCountForUniverseAsync(connStr, universeId.Value, cancellationToken);

                var badges = await GamesRepository.GetBadgesForPlaceAsync(connStr, id, cancellationToken);
                if (badges.Count > 0)
                {
                    var badgeIds = badges.Select(b => b.AssetId).ToList();
                    var badgeStats = await GamesRepository.GetBadgeStatsAsync(connStr, badgeIds, cancellationToken,
                        _configuration.GetValue<bool>("Badge:SubtractCreatorFromTotalWon"));
                    foreach (var badge in badges)
                    {
                        if (badgeStats.TryGetValue(badge.AssetId, out var bs))
                        {
                            badge.TotalWon = bs.TotalWon;
                            badge.WonYesterday = bs.WonYesterday;
                        }
                        badge.Rarity = GamesRepository.GetRarityString(badge.TotalWon, universe.VisitCount);
                    }
                }
...
<script>
    ...
    Roblox.Voting.Initialize();
        ...
        [AllowAnonymous]
        [HttpGet("GetGamePassesInnerPartial")]
        public async Task<IActionResult> GetGamePassesInnerPartial(
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var passes = await GamesRepository.GetGamePassesForUniverseAsync(
                    connStr, universeId.Value, startIndex, maxRows, cancellationToken);
                ...
                {
                    pass.SellerName = await UserQueries.GetUserNameByIdAsync(connStr, pass.OwnerUserId, cancellationToken).ConfigureAwait(false) ?? "Unknown";
                    if (currentUserId > 0)
                    {
                        pass.IsOwned = await userAssetsRepo.UserOwnsAssetAsync(connStr, currentUserId, pass.AssetId, cancellationToken).ConfigureAwait(false);
                    }
                }

                return PartialView("~/Views/Games/GamePassStoreCard.cshtml", passes);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error loading game passes for place {PlaceId}", placeId);
                return Content("");
            }
        }
