...
namespace RobloxWebserver.Controllers
{
    ...
    public sealed class PlacesController : Controller
    {
        ...
        [HttpPost("places/{id}/upload")]
        [Authorize]
        public async Task<IActionResult> UploadPlace(long id, IFormFile file)
        {
            try
            {
                var userIdClaim = User.FindFirst(ClaimTypes.NameIdentifier);
                ...
                if (!string.Equals(System.IO.Path.GetExtension(file.FileName), ".rbxl", StringComparison.OrdinalIgnoreCase))
                {
                    return Json(new { success = false, message = "Invalid file type. Please upload a .rbxl place file." });
                }
                ...
                {
                    return Json(new { success = false, message = "Assets directory not configured" });
                }

                var newHash = HashingUtilities.GenerateFileHash(fileBytes);

                var (success, error) = await GamesRepository.ReplacePlaceAssetAsync(
                    connectionString, assetsDirectory, id, fileBytes, HttpContext.RequestAborted, _assetService);

                if (!success)
                {
                    return Json(new { success = false, message = error ?? "Failed to replace place asset" });
                }
                ...
                _ = Task.Run(async () =>
                {
                    try
                    {
                        var assetRecord = await _assetRepository.GetAssetByIdAsync(connectionString, id);
                        if (assetRecord == null) return;
                        var contentHash = assetRecord.ContentHash;
                        if (string.IsNullOrWhiteSpace(contentHash)) return;

                        await PlaceThumbnail.GeneratePlaceThumbnailAsync(
                            _thumbnailService, connectionString, id, contentHash, baseUrl,
                    ...
                    {
                        Console.WriteLine($"[ThumbnailGen ERROR] Place {id}: {ex.GetType().Name}: {ex.Message}");
                    }
                });

                return Json(new { success = true, message = "Place uploaded successfully", newHash });
            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = "An error occurred while uploading the place. Please try again." });
            }
        }
