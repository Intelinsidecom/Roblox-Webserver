...
public class ThumbnailsController : ControllerBase
{
    ...
    public async Task<IActionResult> AssetThumbnailJson([FromQuery] long assetId, [FromQuery] int? width, [FromQuery] int? height, [FromQuery] string? format, [FromQuery] bool ignoreAssetMedia = false, [FromQuery] bool returnAutoGenerated = false, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<IActionResult> Asset([FromQuery] long assetId, [FromQuery] int? width, [FromQuery] int? height, [FromQuery] string? format, [FromQuery] bool ignoreAssetMedia = false, [FromQuery] bool returnAutoGenerated = false, CancellationToken cancellationToken = default)
    {
        if (assetId <= 0) return BadRequest(new { error = "assetId is required" });
        var connStr = _configuration.GetConnectionString("Default");
        if (string.IsNullOrWhiteSpace(connStr))
        ...
        if (returnAutoGenerated)
        {
            ...
            else
            {
                ...
                {
                    var baseUrl = GetCdnBaseUrl();
                    await PlaceThumbnail.GeneratePlaceThumbnailAsync(_thumbnailService, connStr, assetId, asset.ContentHash ?? "", baseUrl);
