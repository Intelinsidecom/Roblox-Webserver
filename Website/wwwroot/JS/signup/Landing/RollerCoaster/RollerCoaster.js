...
function MoveMagicLine(n) {
    ...
    if (t.length != 0 && (!t.is(":animated") || n)) {
        ...
        var whatsFreebloxiaOffset = $("#WhatsFreebloxiaContainer").offset();
        var navbarHeight = $(".navbar-landing").height() || 0;
        if (whatsFreebloxiaOffset && u < whatsFreebloxiaOffset.top - navbarHeight) {
            if (i = $("#PlayLink").position().left, r = $("#PlayLink").width(), t.data("curtab") == 1 && !n) return;
            t.data("curtab", 1)
        } else {
            var freebloxiaDeviceOffset = $("#FreebloxiaDeviceText").offset();
            if (freebloxiaDeviceOffset && u < freebloxiaDeviceOffset.top - navbarHeight) {
