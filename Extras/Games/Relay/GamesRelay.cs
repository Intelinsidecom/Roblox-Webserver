...
namespace Roblox.Games.Relay
{
	...
	public class GamesRelay
	{
		...
		{
			var app = new ServiceHostApp<GamesRelay>();
		...
		private static void HostApplicationOpening(object sender, EventArgs e)
		{
			...
			if (string.IsNullOrEmpty(Properties.Settings.Default.GameServiceUrl))
				return;
		...
		{
			ReadRccVersion();
			_Logger.LifecycleEvent("RequestDispatches. Game Service Url = {0}, RccVersion = {1}", Properties.Settings.Default.GameServiceUrl, _RccVersion);
			var address = $"{(Properties.Settings.Default.GameServiceUrl)}/v1.0/GetDispatch?apiKey={(Properties.Settings.Default.GamesRelayApiKey)}";
			while (_IsRunning)
			{
				try
				{
				...
				Thread.Sleep(Properties.Settings.Default.RequestDispatchInterval);
			}
		}
		...
		private static readonly ILogger _Logger = new Logger(Properties.Settings.Default.LogName, () => Properties.Settings.Default.LogLevel)
		{
			LogToConsole = true,
			LogThreadID = true
		};
		private static readonly RccServiceArbiter _RccService = new RccServiceArbiter(Properties.Settings.Default.ArbiterEndpoint);
